 ...
 **/ 
 ...
 Fix bugs: 
    - The ENSEMBL ID could be recognized by EN* not __EN*
    - chekc if the transcript id in the gene_position
 Author: Yuelong CHEN
 ...
 **/
 void multi_gene(vector<Alignment> this_al, const map<string, Position> & gene_positions, int gap_size){
   ...
   for(int a=0; a<regions.size(); a++){
     ...
     if(regex_search(trans_id,m,regex("(EN[^_]*)"))){ //Assumed ENSEMBL annotation naming here //EN*
       gene_names.push_back(m[1].str());
       if(gene_positions.count(m[1].str())>0){
        gene_name_lookup[trans_id]=gene_positions.at(m[1].str()).gene;
       }
     } else {
       gene_names.push_back(trans_id);
       if(gene_positions.count(trans_id)>0){
